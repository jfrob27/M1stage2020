PRO POWSPEC_K, a_in, reso, tab_k, spec_k, APODIZE=radius, $
...
IF not keyword_set(a_in) THEN BEGIN
    print,'POWSPEC_K, a_in, reso, tab_k, spec_k, APODIZE=radius, $' 
    print, ' GO_ABOVE_NYQUIST=go_above_nyquist, med_k=med_k, sig_k=sig_k'
    goto, closing
ENDIF

sz=SIZE(a_in)
IF sz(1) NE sz(2) THEN BEGIN
    print,'POWSPEC_K/FATAL : Input is not a square map' 
    goto, closing
ENDIF
...
IF(KEYWORD_SET(radius) ) THEN BEGIN
    IF(radius GT 1.) THEN BEGIN
        print, 'POWSPEC/FATAL : Apodizing radius must be ' + $
          'smaller than and 1' 
        goto, closing
    ENDIF
