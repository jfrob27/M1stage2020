...
acrres = dblarr(2*lag_max+1, 2*lag_max+1)
...
for j=-lag_max, lag_max do begin 
    for i=-lag_max, lag_max do begin
        ...
        if (j ge 0) then begin 
            ...
            jval2 = [-j, taille(2)-1] 
        endelse 
        im1 = image1(ival1(0):ival1(1),jval1(0):jval1(1)) 
        im2 = image2(ival2(0):ival2(1),jval2(0):jval2(1)) 
        ind = where(im1 ne indef and im2 ne indef, nbpoint) 
        if (nbpoint gt 0) then begin
            acrres(lag_max+i, lag_max+j) = total( (im1(ind) - moy1)* (im2(ind) - moy2) )
            nbpoint2d(lag_max+i,lag_max+j) = nbpoint 
        endif else nbpoint2d(lag_max+i,lag_max+j) = 1
    endfor 
    print, "j: ", j
endfor
