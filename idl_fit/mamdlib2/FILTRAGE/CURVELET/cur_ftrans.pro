...
NxCur = (size(Cur))[1]
Ns = fix(Cur[0])
TabNbrAnglePerScale = fix(Cur[1:Ns])
NbrBand = total( TabNbrAnglePerScale )
TabDepRe = fltarr(NbrBand)
TabEndRe = fltarr(NbrBand)
TabDepIm = fltarr(NbrBand)
TabEndIm = fltarr(NbrBand)
TabBandNx = fltarr(NbrBand)
TabBandNy = fltarr(NbrBand)
...
for b=0,TabNbrAnglePerScale[s]-1 do begin
  ...
  TabDepRe[indband] = ind+2
  TabEndRe[indband] = ind+1+Nx*Ny
  BandRe =  reform(Cur[ TabDepRe[indband]:TabEndRe[indband]], TabBandNx[indband], TabBandNy[indband])
  ind = ind + 2 + Nx*Ny
  if Real NE 1 then begin
    TabDepIm[indband] = ind
    TabEndIm[indband] = ind+Nx*Ny-1
    ind = ind + Nx*Ny 
    BandIm =  reform(Cur[ TabDepIm[indband]:TabEndIm[indband]], TabBandNx[indband], TabBandNy[indband])
  end
...
for b=0,CurTrans.TabNbrAnglePerScale[s]-1 do begin
  ...
  if CurTrans.Real NE 1 then BEGIN
     ...
     print, "BAND " , indband,  TabSigRe[indband], TabSigIm[indband]
  END else  print, "BAND " , indband,  TabSigRe[indband] 
  indband = indband + 1
end
end
m = max(TabSigRe, pos)
