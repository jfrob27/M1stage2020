FUNCTION LA_TOT, var1, dim=dim, mask=mask
;+ 
; NAME: LA_TOT
; PURPOSE: 
...
;------------------------------------------------------------
 ...
 IF N_PARAMS() LT 1 THEN BEGIN
   PRINT, 'CALLING SEQUENCE: output=LA_TOT(var1, dim=dim, mask=mask)'
   GOTO, CLOSING
 ENDIF
...
; array case
 ...
 CASE dim OF
      0 : BEGIN
            ...
            IF cpt GT 0 THEN $
               output = total(var1(good_values))
          END
      1 : BEGIN
            output = make_array(n_planes, value=la_undef(4))
            ...
            FOR i = 0l, n_planes-1 DO BEGIN
                ...
                IF cpt GT 0 THEN $
                   output(i) = total(plane(good_values))
            ENDFOR
          END
       -1 : BEGIN
            IF nele1 GT 1 THEN BEGIN
               odim = szv1(1:szv1(0)-1) 
               output = make_array(dimension=odim, value=la_undef(4))
            ...
            FOR i = 0l, nele1-1 DO BEGIN
                ...
                IF cpt GT 0 THEN output(i) = total(vector(values))
            ENDFOR
          END
        ELSE : print, 'Irrelevant value for key "dim" :' + CONV_STRING(dim)
  END
;------------------------------------------------------------
; closing
;------------------------------------------------------------
