function syntres2, result, dsigma=dsigma, dcentroid=dcentroid, xrange=xrange, sigrange=sigrange, centrange=centrange, ylog=ylog
...
x = findgen(nbx)*dcentroid+centrange[0]
...
;      nh = total(mgauss(x, [allamp(i), allcent(i), allsig(i)]))
      a = [allamp(i), allcent(i), allsig(i)]
      f = a(0)*exp(-((x-a(1))/a(2))^2/2.)
      output(*,wy) = output(*,wy)+f
   endif
endfor
